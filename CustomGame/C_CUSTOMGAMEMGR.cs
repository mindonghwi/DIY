...
public class C_CUSTOMGAMEMGR : MonoBehaviour
{

    private CustomGame.C_STAGEMGR m_cStageMgr;
    ...
    private void init()
    {
        m_cStageMgr = new CustomGame.C_STAGEMGR();
        ...
        m_cStageMgr.init(m_goMovingPoint, m_goPosibleTile, m_goImposibleTile, m_cPlayer, m_cLoadData.getLoadNode(), PlayerPrefs.GetInt("CustomMapNum"));

        m_cGameCoin.init(m_cStageMgr.getStartCoinPrice());
        m_cGameCoin.FlututionCoin((m_cStageMgr.getEnemyWave().GetComponent<C_ENEMYWAVE>().getStageCount() + 1));

        m_cPlayer.init(m_cStageMgr.getStartResource(), 0, 30);
        ...
        m_cStageMgr.getEnemyWave().GetComponent<C_ENEMYWAVE>().setDifficultyHp(m_cStageMgr.getDiffculty());
        ...
        GameObject.Find("LoadData").GetComponent<C_LOADCUSTOMTOWER>().setSeletedTower(m_cStageMgr.getListTowerSelected());
    ...
    {
        init();
        GameObject goTmpStartPoint = (GameObject)Instantiate(m_cLoadData.getLoadEffect().getEffect(C_LOADEFFECT.E_EFFECT.E_ENEMYBLAST),
            m_cStageMgr.getEnemyWave().transform.position + new Vector3(0.0f, 2.0f, 0.0f), Quaternion.identity);
